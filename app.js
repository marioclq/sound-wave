...
playPauseButton.addEventListener("click", () => {
    ...
    if (isPlaying) {
        ...
        icon.className = 'fas fa-pause'; // Cambia el ícono a "pause"
    } else {
        audioCtx.suspend();
...
document.getElementById('soundToggle').addEventListener('change', function() {
    const volumeIcon = document.querySelector('.card-body .col-auto i');
    if (this.checked) {
        volumeIcon.className = 'fas fa-volume-up fa-2x text-warning';
    } else {
        volumeIcon.className = 'fas fa-volume-off fa-2x text-gray-300';
    }
});
